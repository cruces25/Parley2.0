...
@st.cache_data(ttl=60) # ttl=60 hace que el portal revise si hay cambios cada 60 segundos
def cargar_datos():
    # PEGA AQUÍ TU ENLACE TRANSFORMADO
    URL_GOOGLE_SHEET = "https://docs.google.com/spreadsheets/d/TU_ID_AQUÍ/export?format=xlsx"
    
    try:
        nomina = pd.read_excel(URL_GOOGLE_SHEET, sheet_name="PLANILLA_CONTROL", header=2)
        cuadros = pd.read_excel(URL_GOOGLE_SHEET, sheet_name="CUADROS_PARLAY", header=1)
